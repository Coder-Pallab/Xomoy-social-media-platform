...
const StoryViewer = ({ viewStory, setViewStory }) => {
    ...
    const renderContent = () => {
        switch (viewStory.media_type) {
            ...
            case 'video':
                return (
                    <video onEnded={() => setViewStory(null)} src={viewStory.media_url} className='max-h-screen' alt="" controls autoPlay />
                )
                break;
    ...
    return (
        <div className='fixed inset-0 h-screen bg-black bg-opacity-90 z-110 flex items-center justify-center' style={{ backgroundColor: viewStory.media_type === 'text' ? viewStory.background_color : '#000000' }}>
            ...
            {/* Content Wrapper */}
            <div className='max-w-[990vw] max-h-[90vh] flex items-center justify-center'>
                {renderContent()}
            </div>
        </div>
    )
