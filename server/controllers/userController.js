...
export const updateUserData = async (req, res) => {
    try {
        ...
        if (tempUser.username !== username) {
            ...
            if (user) {
                // We'll not change the username if it is already taken
...
export const followUser = async (req, res) => {
    try {
        ...
        if (user.following.includes(id)) {
            return res.json({ success: false, message: "yor are already following this user..." })
        }
...
export const sendConnectionRequest = async (req, res) => {
    try {
        ...
        if(connectionRequests.length >= 20){
            res.json({ success: false, message: 'You have sent more than 20 requests in the last 24 hours'})
        }
